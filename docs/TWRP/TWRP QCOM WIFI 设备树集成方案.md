...
- [TWRP Recovery WiFi 设备树集成指南](#twrp-recovery-wifi-设备树集成指南)
  ...
  - [十、WiFi 连接与 DHCP](#十wifi-连接与-dhcp)
    ...
    - [10.2.1 方案 A：toybox dhcp + ip route](#1021-方案-atoybox-dhcp--ip-route)
    - [10.2.2 方案 B：dhcpcd](#1022-方案-bdhcpcd)
    - [10.2 ping 权限问题](#102-ping-权限问题)
  - [十一、关键背景知识](#十一关键背景知识)
  ...
  - [十二、调试手段](#十二调试手段)
    ...
    - [排查 TWRP 卡第一屏（进不去主界面）](#排查-twrp-卡第一屏进不去主界面)
      - [第一步：确认 recovery 进程是否在跑](#第一步确认-recovery-进程是否在跑)
      - [第二步：找阻塞点](#第二步找阻塞点)
      - [第三步：找 HAL 未启动的原因](#第三步找-hal-未启动的原因)
  - [十三、排查案例](#十三排查案例)
  ...
  - [十四、wpa\_cli /tmp 权限问题（自编译版专有）](#十四wpa_cli-tmp-权限问题自编译版专有)
    ...
    - [14.4 两种修复方案](#144-两种修复方案)
      - [方案一（已采用）：chown /tmp](#方案一已采用chown-tmp)
      - [方案二：用 `-s` 参数指定 wpa\_cli 临时 socket 目录](#方案二用--s-参数指定-wpa_cli-临时-socket-目录)
    - [14.5 影响范围](#145-影响范围)
...
详细代码在 [wpa_supplicant_8](https://github.com/AuroraRecoveryProject/wpa_supplicant_8)/[android_vendor_twrp](https://github.com/AuroraRecoveryProject/android_vendor_twrp)，但我我特么忘了这个库从哪儿 clone 下来的了，应该是从安卓某个分支
...
# 见 https://github.com/AuroraRecoveryProject/aurora_recovery/blob/main/docs/TWRP/TWRP%20QCOM%20WIFI%20%E6%89%8B%E5%8A%A8%E5%90%AF%E5%8A%A8%E5%85%A8%E6%B5%81%E7%A8%8B.md
...
| Snapdragon 8 Elite（sun） | peach | `b0000000.qcom,cnss-peach` |
| Snapdragon 8 Elite Gen 5 (canoe) | peach | `b0000000.qcom,cnss-peach` |
...
下面先给出 OEM 与 Pures 方案共用的骨架，再给出各自需要替换的差异块。
...
### 10.2.1 方案 A：toybox dhcp + ip route
...
**解决方案**：`init.recovery.qcom.rc` 在 `twrp.modules.loaded=true` 时会 `umount /vendor`，此后 `/vendor/odm` 消失，循环链自动断开。在 WiFi 驱动加载完成后（`lauriewifi.driver.ready=true`，此时 `/vendor` 已 umount），重启 servicemanager，则：
